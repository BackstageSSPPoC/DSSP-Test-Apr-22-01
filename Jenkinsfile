...
//     environment {
//         APP_NAME = "DSSP-Test-Apr-22-01".toLowerCase()
...
//     environment {
//         APP_NAME = "DSSP-Test-Apr-22-01".toLowerCase().trim()
//         DOCKER_IMAGE = "chaitanyapandeygspann/${APP_NAME}"
//         DOCKER_TAG = "1.0.${BUILD_NUMBER}"
//         IMAGE_TAG = "${DOCKER_IMAGE}:${DOCKER_TAG}"
//         GITOPS_REPO = "https://github.com/BackstageSSPPoC/k8s-manifests.git"
...
pipeline {
    ...
    stages {
        ...
        // 🔥 IMPORTANT: Skip build if branch just created (no new commits)
        stage('Skip Build on Branch Creation') {
            when {
                not {
                    anyOf {
                        branch 'main'
            ...
            steps {
                script {
                    def commitDiff = sh(
                        script: "git rev-list --count HEAD ^origin/main || echo 0",
                        returnStdout: true
                    ).trim()
    
                    echo "Commit difference from main: ${commitDiff}"
    
                    if (commitDiff == "0") {
                        echo "Branch just created → Skipping CI"
                        currentBuild.result = 'NOT_BUILT'
                        error("Skipping build: no new commits")
                    }
                }
            }
        }
